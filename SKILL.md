...
在 Sol 已确认任务规格后，Luna 可以完成隔离实现、测试或只读检查；Sol 最多协调 3 个独立 Luna 子 Agent，并保留综合、集成和最终复核责任。卖家在本地表单主动填写的基础资料可用于当前本地计算；但从外部访问或处理敏感卖家资料、外部 MCP/API、账户变更或可执行广告动作只能由 Sol 在获得用户当次明确批准后进行；在此之前只能准备方案或不可执行草案。
